...
import { db, withDirectClient } from '@/lib/db';
...
export async function POST() {
  try {
    // Use direct client for transactions (bypasses pooler limits)
    const result = await withDirectClient(async (client) => {
      // Use transaction to batch all operations into a single connection
      return client.$transaction(async (tx) => {
        ...
        await tx.kenyaLocation.deleteMany();
        await tx.communityReview.deleteMany();
        await tx.exchangeRate.deleteMany();
        await tx.communityPhoto.deleteMany();
        await tx.nextDrop.deleteMany();
        await tx.subscriber.deleteMany();
        await tx.orderItem.deleteMany();
        await tx.order.deleteMany();
        await tx.cartItem.deleteMany();
        await tx.loyalty.deleteMany();
        await tx.review.deleteMany();
        await tx.customer.deleteMany();
        await tx.product.deleteMany();
        await tx.category.deleteMany();
        await tx.storeSettings.deleteMany();
        await tx.socialHandle.deleteMany();
        ...
        await tx.socialHandle.createMany({
          data: [
        ...
        await tx.communityPhoto.createMany({
          data: [
        ...
        await tx.kenyaLocation.createMany({
          data: [
        ...
        return { products: productsData.length, reviews: reviewsData.length };
      });
    });
